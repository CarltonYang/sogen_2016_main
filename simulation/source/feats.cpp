...
void osc_features_ant (sim_data& sd, input_params& ip, features& wtfeat, char* filename_feats, con_levels& cl, mutant_data& md, int start_line, int end_line, int start_col, int end_col, int set_num) {
	...
	char* str_set_num = new char[(strlen_set_num + 1)];
	...
	for (int i = 0; i < 4; i++) {
		...
		if (ip.ant_features) {
			for (int j = 0; j < NUM_FEATURES; j++) {
				char* filename = new char[strlen(filename_feats) + strlen("set_") + strlen_set_num + 1 + strlen(feat_names[j]) + 1 + strlen(concs[i]) + strlen("_ant.feats") + 1];
	...
	*/
    if (md.index== MUTANT_WILDTYPE || md.index== MUTANT_DELTA){
        ...
        for (int i = 0; i < num_genes; i++) {
            if (ip.post_features) {
                for (int j = 0; j < NUM_FEATURES; j++) {
                    char* filename = new char[strlen(filename_feats) + strlen("set_") + strlen_set_num + 1 + strlen(feat_names[j]) + 1 + strlen(concs[i]) + strlen("_post.feats") + 1];
