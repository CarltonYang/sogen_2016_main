...
double simulate_param_set (int set_num, input_params& ip, sim_data& sd, rates_static& rs, rates_dynamic& rs_d, con_levels cls[], con_levels baby_cls[], mutant_data mds[], ofstream* file_passed, ofstream* file_scores, char** dirnames_cons, ofstream* file_features, ofstream* file_conditions) {
    ...
    int num_passed = 0;
    double scores[NUM_SECTIONS * NUM_MUTANTS *1] = {0};
...
void launchkernel(int cells_total,sim_data& sd, params p_darray[], array2D<double> rates_active_darray[],con_levels baby_cls_darray[], int baby_j,int j,int time_prev,bool past_induction[], bool past_recovery[]){
	...
	CUDA_ERRCHK(cudaDeviceSetLimit(cudaLimitStackSize, 65536));
	//Run kernel
...
__device__ inline void con_protein_her (cp_args& a, cph_indices i) {
    ...
    double temp4=  c[i.con_mrna][td][a.old_cells[i.old_cell]];
    
	if (isnan(temp4)|| temp4!=temp4){
		//c[i.con_mrna][td][a.old_cells[i.old_cell]]=0;
		//printf("nan found: temp4: %f; i.con_mrna %d; td: %d; a.old_cells[i.old_cell]: %d",temp4, i.con_mrna,td, a.old_cells[i.old_cell] );
		//asm("trap;");
	}
...
__device__ inline void con_dimer (cd_args& a, int con, int offset, cd_indices i) {
	...
	double temp1 = c[con][tp][cell];
	double temp2 = r[i.rate_association + offset][cell];
	double temp3 = c[i.con_protein][tp][cell] ;
	double temp4 = c[i.con_protein + con_offset][tp][cell];
	/*
	if (isnan(temp1)|| temp1!=temp1){
		printf("nan found: temp1: %f ",temp1 );
		asm("trap;");
	}
	
	if (isnan(temp2)|| temp2!=temp2){
		printf("nan found: temp2: %f ",temp2 );
		asm("trap;");
	}
	
	if (isnan(temp4)|| temp4!=temp4){
		printf("nan found: temp4: %f ",temp4 );
		asm("trap;");
	}
	
	if (isnan(temp3)|| temp3!=temp3){
		printf("nan found: temp3: %f ",temp3 );
		asm("trap;");
	}*/
}
...
__device__ inline double transcription_mespa (array2D<double>& rs, con_levels& cl, int time, int cell, double avgpd, double ms, double oe, int section) {
	...
	double tempxx=ms * (oe + (tdelta) / (tdelta + rs[NS1][cell] * SQUARE(th1h1)  + SQUARE(tmespbmespb)));
	if(isnan(tempxx)|| tempxx!=tempxx){
		return 0;
		//printf("nan found: tempxx: %f; th1h1: %f; tmespbmepsb: %f; tdelta: %f",tempxx, th1h1, tmespbmespb, tdelta );
		//asm("trap;");
	}
...
__device__ inline double transcription_mespb (array2D<double>& rs, con_levels& cl, int time, int cell, double avgpd, double ms, double oe) {
	...
	/*
	if (isnan(tmespamespa)|| tmespamespa!=tmespamespa){
		printf("nan found: tmespamespa, cl: %f , rs: %f ",cl.cons[CPMESPAMESPA][time][cell],(rs[RCRITPMESPAMESPA][cell]) );
		asm("trap;");
	}

    if (isnan(tmespamespb)|| tmespamespb!=tmespamespb){
		printf("nan found: tmespamespa, cl: %f , rs: %f ",cl.cons[CPMESPAMESPB][time][cell],(rs[RCRITPMESPAMESPB][cell]) );
		asm("trap;");
	}
	if (isnan(tmespbmespb)|| tmespbmespb!=tmespbmespb){
		printf("nan found: tmespbmespb, cl: %f , rs: %f ",cl.cons[CPMESPBMESPB][time][cell],(rs[RCRITPMESPBMESPB][cell]) );
