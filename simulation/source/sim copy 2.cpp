    ...
    for (j = sd.time_start, baby_j = 0; j < sd.time_end; j++, baby_j = WRAP(baby_j + 1, sd.max_delay_size)) {
        ...
        for (int k = 0; k < sd.cells_total; k++) {
            if (sd.width_current == sd.width_total || k % sd.width_total <= sd.active_start) { // Compute only existing (i.e. already grown) cells
                ...
                if (md.index==MUTANT_MESPBOVER &&  stc.time_cur==387 && stc.cell==177){//cout<<cl.cons[CPMESPAMESPA][2527][177]<<" "<<cl.cons[CPMESPAMESPB][2527][177]<<" "<<cl.cons[CPMESPBMESPB][2527][177]<<endl;
                }
        ...
        if (j==anterior_time(sd,31674) && md.index==MUTANT_MESPBOVER){
            for (int w=0; w< sd.cells_total;w++){
                cout<< cl.cons[CMMESPB][j][w]<< " ";
            }
            cout<<endl;
        }
    }
...
inline void con_dimer (cd_args& a, int con, int offset, cd_indices i) {
    ...
    if (isinf(c[con][tc][cell])&& !isinf(c[con][tp][cell])){cout<<"IN CONDIMER2: "<<tc<<" | "<<c[con][tp][cell]<<" "<< a.sd.step_size * r[i.rate_association + offset][cell] * c[i.con_protein][tp][cell] * c[i.con_protein + con_offset][tp][cell]<<" "<<r[i.rate_dissociation + offset][cell] * c[con][tp][cell]<<" "<<r[i.rate_degradation + offset][cell] * c[con][tp][cell] <<endl; }
    
    //if (con==CPMESPBMESPB&&tc== 2527 && cell ==177){cout<<"IN CONDIMER: "<<c[CPMESPAMESPA][2527][177]<<" "<< c[CPMESPAMESPB][2527][177]<< " "<<c[CPMESPBMESPB][2527][177]<<" | "<<c[con][tp][cell]<<" "<< a.sd.step_size * r[i.rate_association + offset][cell] * c[i.con_protein][tp][cell] * c[i.con_protein + con_offset][tp][cell]<<" "<<r[i.rate_dissociation + offset][cell] * c[con][tp][cell]<<" "<<r[i.rate_degradation + offset][cell] * c[con][tp][cell] <<endl;}
...
void mRNA_synthesis (sim_data& sd, double** rs, con_levels& cl, st_context& stc, int old_cells_mrna[], mutant_data& md, bool past_induction, bool past_recovery, double& avg_delta,double& avg_rest ) {
    ...
    for (int j = 0; j < NUM_INDICES; j++) {
        ...
        if (j == IMMESPA && sd.section == SEC_ANT) {
            ...
            mtrans= ms * (oe + (1 + tdelta) / (1 + tdelta + SQUARE(tmespamespa) + SQUARE(tmespamespb)+SQUARE(tmespbmespb)));
            if (md.index==MUTANT_MESPBOVER && (CMH1+j == CMMESPB &&( stc.time_cur>=1465&& stc.time_cur<= 1470)&& stc.cell==177)){cout<<mtrans<<" "<< time<<" "<<cell<<" "<<avgpd<<" "<<ms<<" "<<oe<< " "<< tdelta<< " "<<cl.cons[CPMESPAMESPA][time][cell]<<" "<<(rs[RCRITPMESPAMESPA][cell])<<" "<< cl.cons[CPMESPAMESPB][time][cell]<<" "<< (rs[RCRITPMESPAMESPB][cell])<<" "<< cl.cons[CPMESPBMESPB][time][cell]<<" "<<(rs[RCRITPMESPBMESPB][cell])<<endl;}
            
            //if (md.index==MUTANT_MESPBOVER && (CMH1+j == CMMESPB  && stc.cell==177)){cout<<mtrans<<" "<< WRAP(stc.time_cur - delays[j], sd.max_delay_size)<<" "<<old_cells_mrna[IMH1 + j]<<" "<<avgpd<<" "<<rs[RMSH1 + j][stc.cell]<<" "<<oe<< endl;}
        } else {
        ...
        if (md.index==MUTANT_MESPBOVER && (CMH1+j == CMMESPB  && stc.cell==177&& ( stc.time_cur>=1465&& stc.time_cur<= 1470))){
            if (isnan(cl.cons[CMH1 +j][stc.time_cur][stc.cell])){
                cout<<" PASSED"<<CMH1 +j<<" "<<stc.time_cur<<" "<<stc.cell<<"| "<<cl.cons[CMH1 + j][stc.time_cur][stc.cell]<<" "<<cl.cons[CMH1 + j][stc.time_prev][stc.cell]<<" "<<sd.step_size <<" "<< mtrans << " "<<rs[RMDH1 + j][stc.cell] << " "<<cl.cons[CMH1 + j][stc.time_prev][stc.cell]<<endl;
            }
        }
    }
}
